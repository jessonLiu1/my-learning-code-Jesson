...
int sumStack(Stack<int> s) {
    int total = 0; //change the initial value of 'total'
    while (!s.isEmpty()) {
        total += s.pop();
    }
    return total;
...
STUDENT_TEST("duplicateNegatives, a negative at last") {
    Queue<int> q = {5, -7};
    Queue<int> expected = {5, -7, -7};

    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}

STUDENT_TEST("duplicateNegatives, a negative at first") {
    Queue<int> q = {-7, 5};
    Queue<int> expected = {-7, -7, 5};
...
STUDENT_TEST("duplicateNegatives, input has only one negative") {
    Queue<int> q = {-7};
